...
function extractRequest(superTest) {
  ...
  if (superTest._data) { fields.data = superTest._data; }
  if(superTest.requestValidator){ fields.requestValidator = superTest.requestValidator; }
  if(superTest.requestHeaderValidator){ fields.requestHeaderValidator = superTest.requestHeaderValidator; }
  if(superTest.originalUrl){ fields.originalUrl = superTest.originalUrl; }
  return fields;
}
...
function extractResponse(superTest, response) {
  ...
  if(superTest.respondValidator){ fields.respondValidator = superTest.respondValidator; }
  return fields;
}
